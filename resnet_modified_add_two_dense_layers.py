...
class ResNet: 
    def __init__(self, path, x_tr, y_tr, x_v, y_v, subtract_pixel_mean = True, version=1, n=3):
        ...
        if version == 2:
            self.model = resnet_v2(input_shape=x_tr.shape[1:], depth=depth, num_classes=y_v.shape[1])
        else:
            self.model = resnet_v1(input_shape=x_tr.shape[1:], depth=depth, num_classes=y_v.shape[1])
        base = Model(inputs = self.model.inputs, outputs = self.model.layers[-2].output, name=f"base_2nd_to_last")
        x = keras.Input(shape=base.input_shape[1:], name="input_2nd_to_last")

        y = Dense(10, activation='relu', kernel_initializer='he_normal', name="dense_attached")(base(x))
        y = Dense(10, name="dense_attached_second")(y)
