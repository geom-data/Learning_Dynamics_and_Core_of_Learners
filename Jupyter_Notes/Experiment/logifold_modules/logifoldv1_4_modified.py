...
class Logifold:
    def __init__(self, target : int, dataType : str ="numpy",
                ...
                path : str =None, path_for_cache : str =None, load_val_path : bool =False, load_tr_path : bool =False,
    ...
    def add(self,model,model_path : str = None, key : tuple = None ,filetype : str = 'keras',target=None,
        ...
        - If key is None and model_path is provided, key is derived from model_path (suffix removed).
        - The original model_path is recorded in description and in a structured 'spec for loading model'.
        """
        if key is None:
            if model_path is None:
                raise ValueError("Provide either 'key' or 'model_path' to derive key.")
            key = (int_from_model_path(model_path),)
        ...
        info = f"{str(model_path) if model_path else '<unknown>'}"
        if model_path is not None and key is None:
            info += f", derived_key={key}"
        if description:
            self.charts[key]['description'] = f"{info} | source path | {description} "
        else:
            self.charts[key]['description'] = f"{info} | source path | empty description "
        self.charts[key]['spec for loading model'] = None
        if fuzDom is None:
    ...
    def modelPath(self,key,filetype):
        ...
        for i in key:
            s+='_%03d'%i
        return self.path+'models/'+'model'+s+'.'+filetype
    ...
    def model_source_name(self,keys):
        if len(keys) == 1 and isinstance(keys,list):
            k = keys[0]
            description = self.charts[k]['description']
            if description is None or description.split('|')[0].strip() == '<unknown>':
                return None
            else:
            ...
            for k in keys:
                ...
                if description is None:
                    keys_source_unknown.append(k)
                else:
                    name = description.split('|')[0].strip()
                    if name == '<unknown>':
                        keys_source_unknown.append(k)
                    else:
                        names_source_known.append(name)
            return names_source_known, keys_source_unknown
    ...
    def getFuzDoms(self, x=None, y=None, sample_name : str = '',iniTh=0., step=0.5, 
        ...
        for k in keys:
            ...
            if self.path_for_cache is not None and sample_name != '' and self.model_source_name(k):
                stored_cache = cache_store.ResultStore(self.path_for_cache)
                model_name = self.model_source_name(k)
                predict = stored_cache.get_pred(model_name, sample_name)
                ...
                if predict is None:
                    ...
                    stored_cache.set_pred(model_name, sample_name, predict)    
            else:
                model = self.getModel(k)
                if force_batch == True:
                    predict = self.force_batch_model_predict(model, x_k, batch_size)      
    ...
    def keysInTarget(self, target, keys=None, active=False):
        ...
        for k in keys:
            ...
            if self.charts[k]['target']==target:
                chosen.append(k)
        return chosen
    def __init_raw_preds__(self):
        '''
        Re-initialize raw prediction to None.
        '''
        self.raw_predictions = None
        
    #We include evaluate function into `predict'.
    #Evaluate if `y' is not None
    def predict(self, x, x_name:str, wantAcc=None, maskInWantAcc=None, keys=None, 
                ...
                display_ : bool = True,
                path_for_cache : str = None):
        assert voteBy=='weighAcc' or voteBy=='order'
        if self.raw_predictions is None: self.raw_predictions = {}
        ...
        for gp in keys_gps:
            ...
            thisAns,thisCertPart,thisOriginalAns = self.predictSameFlatTarget(x, x_name, wantAcc=wantAcc[needToProcess], 
                                                                   keys=gp, batch_size=batch_size,
                                                                   force_batch=force_batch, 
                                                                   onlyFineCanVote=onlyFineCanVote, 
                                                                   active=False, verbose=verbose,
                                                                   voteBy=voteBy, path_for_cache=path_for_cache)
        ...
        for n in nextNodes:
            ...
            '''
            fullAns, certPart, pred, certs, out, originalAns, modelAcc, pred_useHist = self.predict(x, x_name,
                                                        wantAcc=wantAcc, 
    ...
    def predictSameFlatTarget(self, x, x_name : str, wantAcc, keys=None, batch_size=128, force_batch=False, 
                onlyFineCanVote=False, active=True, verbose=1,
                voteBy='weighAcc', path_for_cache : str =None):
        ...
        if keys is None:
            keys=self.keys()
        if path_for_cache is not None:
            storage = cache_store.ResultStore(path_for_cache)
            for k in keys:
                model_name = self.model_source_name([k])
                p = storage.get_pred(model_name, x_name)
                if p is not None:
                    self.raw_predictions[(k, x_name)] = p
        ...
        if voteBy=='order':
            ...
            if (keys[0], x_name) not in self.raw_predictions:  
                ...
                self.raw_predictions[(keys[0], x_name)] = pred
                storage.set_pred(self.model_source_name(keys[0]), x_name, pred) 
            else:
                pred = self.raw_predictions[(keys[0], x_name)]
            ...
            for key in keys[1:]:
                if (key, x_name) not in self.raw_predictions:
                    ...
                    if force_batch == True:
                        ...
                        pred = model.predict(x,batch_size=cur_batch_size,verbose=0)
                    self.raw_predictions[(key, x_name)] = pred
                    storage.set_pred(self.model_source_name(key), x_name, pred) 
                else:
                    pred = self.raw_predictions[(key, x_name)]
            ...
            for i in range(len(keys)):
                ...
                assert fuzDom != None
                if (keys[i], x_name) not in self.raw_predictions:
                    ...
                    if keys[i] not in self.raw_predictions:
                        if force_batch == True:
                            pred = self.force_batch_model_predict(model, x, cur_batch_size)      
                        else:
                            pred = model.predict(x,batch_size=cur_batch_size,verbose=0)
                    self.raw_predictions[(keys[i], x_name)] = pred
                else:
                    pred = self.raw_predictions[(keys[i], x_name)]
...
#             print("Vote counting...") 
    ...
    def load(self):
        ...
        modelFiles = [f for f in os.listdir(self.path+'models/') if f.startswith("model")]
        for f in modelFiles:
            fsplit = f.split('_')
            key = ()
...
def _stem_all(path: str) -> str:
    """Strip ALL suffixes: 'path/to/x.keras.keras' -> 'x'."""
    p = Path(path)
    while p.suffix:
        p = p.with_suffix('')
    return p.name
...
def int_from_model_path(model_path: str) -> int:
    """
    Build a tuple key from the filename (no extensions).
    Example file path : '/path/to/n_3_v1_cifar10_2.keras'
    Results:
    _stem_all -> 'n_3_v1_cifar10_2' 
            -> 5939981164063472405
    """
    name = _stem_all(model_path).casefold()

    # 64-bit id from BLAKE2s hash of name
    h = hashlib.blake2s(name.encode('utf-8'), digest_size=8).digest()
    return int.from_bytes(h, 'big')  # e.g., 1029384756...
