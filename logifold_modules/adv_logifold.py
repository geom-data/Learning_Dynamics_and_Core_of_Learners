...
def cache_id_64(key, namespace: bytes = b"lgfd-cache-v1") -> int:
    """
    Deterministic 64-bit ID from an int or tuple[int,...].
    Suitable for filenames, dicts, NumPy int64, DB BIGINT, etc.
    """
    if isinstance(key, tuple):
        payload = namespace + b"|" + b",".join(str(int(x)).encode() for x in key)
    else:
        payload = namespace + b"|" + str(int(key)).encode()
...
class AdvLogifold(Logifold):
    ...
    def _committee_sig_from_keys(self, keys: list) -> Optional[str]:
        ...
        if not known:
            return None
#         known = [str(cache_id_64(key)) for key in keys]
    ...
    def get_entropy_array(
        ...
        if self.path_for_cache is not None:
            cache_root = self.path_for_cache
            storage = cache_store.ResultStore(cache_root)
        ...
        if sample is not None and not have_all:
            for k in keys:
                if (k, sample_name) not in self.raw_predictions:
                    pred = self.getModel(k).predict(sample, verbose=0)
                    self.raw_predictions[(k, sample_name)] = pred
                    storage.set_pred(self.model_source_name(k), sample_name, pred) 
        ...
        known, unknown = self.model_source_name(keys)
        if unknown:
            print(f'unknown from {keys} = {unknown}')
        file_path = storage.set_entropy(known, sample_name, ent)
        
        print('Entropy has been stored to', file_path)
        return ent
